...
-- CONSULTA NO. 3
-- ------------------------------------------------
SELECT datos.Pais,datos.region, datos.depto, datos.Municipio, datos.nombre_partido FROM
(SELECT pa.nombre as Pais, reg.nombre as Region, depto.nombre as Depto, mun.nombre as Municipio, par.nombre_partido, (voto.analfabetos+voto.alfabetos) as Votos FROM r_votos as voto INNER JOIN pais pa ON voto.id_pais = pa.id_pais
INNER JOIN region reg ON voto.id_region = reg.id_region
INNER JOIN departamento depto ON voto.id_departamento = depto.id_departamento
INNER JOIN municipio mun ON voto.id_municipio = mun.id_municipio
INNER JOIN partido par ON voto.id_partido = par.id_partido
GROUP BY pa.nombre, reg.nombre, depto.nombre, mun.nombre, par.nombre_partido
ORDER BY 1,2,3,4) AS Datos INNER JOIN (SELECT datos.pais,datos.region, datos.depto, datos.mun, MAX(datos.votos) as votos FROM (SELECT pa.nombre as pais, reg.nombre as region, depto.nombre as depto, mun.nombre as mun, par.nombre_partido as partido, (voto.analfabetos+voto.alfabetos) as Votos FROM r_votos as voto INNER JOIN pais pa ON voto.id_pais = pa.id_pais
INNER JOIN region reg ON voto.id_region = reg.id_region
INNER JOIN departamento depto ON voto.id_departamento = depto.id_departamento
INNER JOIN municipio mun ON voto.id_municipio = mun.id_municipio
INNER JOIN partido par ON voto.id_partido = par.id_partido
GROUP BY pa.nombre, reg.nombre, depto.nombre, mun.nombre, par.nombre_partido
ORDER BY 1,2,3,4) as datos
GROUP BY datos.pais, datos.mun
ORDER BY 1,2) AS ganadores ON datos.Pais = ganadores.pais AND datos.Municipio = ganadores.mun AND ganadores.votos = datos.votos
;


SELECT DISTINCT pa.nombre, par.nombre_partido FROM r_votos as voto INNER JOIN pais pa ON voto.id_pais = pa.id_pais
INNER JOIN partido par ON voto.id_partido = par.id_partido
INNER JOIN (SELECT datos.Pais,datos.region, datos.depto, datos.Municipio, datos.nombre_partido FROM
(SELECT pa.nombre as Pais, reg.nombre as Region, depto.nombre as Depto, mun.nombre as Municipio, par.nombre_partido, (voto.analfabetos+voto.alfabetos) as Votos FROM r_votos as voto INNER JOIN pais pa ON voto.id_pais = pa.id_pais
INNER JOIN region reg ON voto.id_region = reg.id_region
INNER JOIN departamento depto ON voto.id_departamento = depto.id_departamento
INNER JOIN municipio mun ON voto.id_municipio = mun.id_municipio
...
ORDER BY 1,2,3,4) as datos
GROUP BY datos.pais, datos.mun
ORDER BY 1,2) AS ganadores ON datos.Pais = ganadores.pais AND datos.Municipio = ganadores.mun AND ganadores.votos = datos.votos) as generality ON pa.nombre = generality.pais AND par.nombre_partido = generality.nombre_partido
ORDER BY 1,2
;
